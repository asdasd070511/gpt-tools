...
def _login_for_token(
    ...
    try:
        ...
        # 5. 如果重定向没直接拿到 callback，尝试从 cookie 提取 workspace 继续
        ...
        if ca_continue and "phone" not in ca_continue.lower():
            ...
            for _redir in range(10):
                ...
                if "code=" in next_url and "state=" in next_url:
                    ...
                    return submit_callback_url(callback_url=next_url, s=s)
                current_url = next_url
            ...
            if ca_continue:
                ...
                for _ in range(6):
                    ...
                    if "code" in qs and "state" in qs:
                        ...
                        return submit_callback_url(location, s)
...
def main() -> None:
    ...
    parser.add_argument(
        "--proxy", default=None,
        help="代理地址，如 http://127.0.0.1:7890（留空则使用全局 PROXY_URL）"
    )
    parser.add_argument(
        "--no-proxy", action="store_true",
        help="禁用代理，直连（覆盖全局 PROXY_ENABLED）"
    )
    ...
    def _one_run(idx: int) -> None:
        ...
        try:
            # 决定代理：--no-proxy 强制直连，否则用 --proxy 或全局配置
            use_proxy = None
            if not args.no_proxy:
                use_proxy = args.proxy or (PROXY_URL if PROXY_ENABLED else None)
            token_json = run(use_proxy)
            if token_json:
